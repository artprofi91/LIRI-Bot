...
## commands ##
The `node liri.js commands` will show you all commands that you can use in the Liri-Bot.
...
For example, `node liri.js spotify-this-song favorite song` will run a Spotify API search for songs with the title "Favorite Song" and log the first result.
...
For example, `node liri.js my-tweets` will log up to 20 tweets from the account.
...
For example, `node liri.js movie-this Suits` will run a OMDB API search for movies with the title "Suits" and log the first result.
...
The `node liri.js do-what-it-says` command cannot run itself. If `do-what-it-says` is in `random.txt`, the other commands will run, but instances of the `do-what-it-says` command will not run.
